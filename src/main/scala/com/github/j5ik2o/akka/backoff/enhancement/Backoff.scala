...
trait BackoffOptions {
  ...
  def withOnStartChildHandler(onStartChildHandler: Option[Throwable] => Unit): BackoffOptions

  def withOnStopChildHandler(onStopChildHandler: () => Unit): BackoffOptions
...
private final case class BackoffOptionsImpl(
  ...
  override def withOnStartChildHandler(onStartChildHandler: (Option[Throwable]) => Unit): BackoffOptions =
    copy(onStartChildHandler = onStartChildHandler)

  override def withOnStopChildHandler(onStopChildHandler: () => Unit): BackoffOptions =
